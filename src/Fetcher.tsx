...
import styles from './fetcher.module.scss';
import {CheckIcon} from './Icon';
import {IFetcherOption, TOption} from './types';
...
const Fetcher = <T extends unknown>({
     ...
     * @param row
     */
    const renderOptionsButton = (row: IFetcherOption<T>) => {
    ...
    const renderOptions = useCallback(() => {
        ...
        if(!elOptions || elOptions?.length === 0){
            ...
            >
                <div className={clsx(styles.listItemText, styles.listItemTextNoData)}>{i18n('com.fetcher.noData', {def: 'No data'})}</div>
            </div>);
